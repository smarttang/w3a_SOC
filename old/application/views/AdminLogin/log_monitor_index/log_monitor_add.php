...
<div id="pad-wrapper" class="users-list">
	...
	<div class="control-group">
	  ...
	  <div class="controls">
	    <select id="task_user_id" name="task_user_id" style="height:26px" class="input-medium">
	      <?php foreach($user_id as $user_id_item):
	      	 echo '<option value="'.$user_id_item['id'].'">'.$user_id_item['name'].'</option>';
		 endforeach;
	       ?>
	    </select>
	  </div>
	...
	<!-- Select Basic -->
	<div class="control-group">
	  <label class="control-label" for="task_switch">任务初始状态</label>
	  <div class="controls">
	    <select id="task_switch" name="task_switch" style="height:26px" class="input-medium">
	...
	<div class="control-group">
	  ...
	  <div class="controls">
	    <select id="task_service_type" name="task_service_type" style="height:29px" class="input-medium">
	     <?php foreach($log_type as $log_type_item):
	...
	<div class="control-group">
	  <div class="controls">
	    ...
	    <button id="button2id" name="button2id" class="btn btn-info" onclick="goToUrl('/log_monitor_index/log_monitor_list')">返回</button>
	  </div>
	</div>
	</fieldset>
	</div>
	...
	var task_s=$('#task_switch').val();
        var task_t=$('#task_service_type').val();
        if(checkFrom(task_u)){
                $.ajax({
                        type:'POST',
                        url:'<?=site_url().'/Admin/log_monitor_index/log_monitor_insert'?>',
                        data:{task_name:task_n,task_url:task_u,task_user_id:task_u_id,task_switch:task_s,task_service_type:task_t},
                        dataType:'text',
                        timeout:15000,
                        success:function(){
